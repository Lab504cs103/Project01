...
if ($weekfirst=="" && !empty($PHP_AUTH_USER))            // 不是從連結而來
      ...
      <option >選擇要取消預約的教室</option>
        <?
          ...
          while ($row = mysql_fetch_row($result)) {
                    list($room) = $row;
                    echo "<option value=\"delete_fix_reserve.php?classroom=$room&nextfirst=$nextfirst\">
                 $room</option>";
           }
           mysql_close ($link);
        ?>
...
<form metohd="post" action="delete_fix_reserve.php">
<div align="center">

<table border="0" width="80%" cellpadding="2" style="border-collapse: collapse" bordercolor="#111111" cellspacing="5">
<tr>
<td>
<p align="left"><font color="#345998"><b><font size="2">管理權模式會顯示所有預約情形，請小心刪除。</font>
<br>
</p>
</td>

<td>
    《<a href="delete_fix_reserve.php?classroom=<?echo $classroom;?>&weekfirst=<?echo date("Y-m-d",mktime(0,0,0,$tok[1],$tok[2]-7,$tok[0]))?>"><font color="#000080">上一週</font></a>》
    《<a href="delete_fix_reserve.php?classroom=<?echo $classroom;?>&weekfirst=<?echo date("Y-m-d",mktime(0,0,0,$tok[1],$tok[2]+7,$tok[0]))?>"><font color="#000080">下一週</font></a>》
</td>
